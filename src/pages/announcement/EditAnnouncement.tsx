...
import { getAnnouncementDetailsApi, updateAnnouncementApi } from "@/apis/announcement";
...
export default function EditAnnouncement() {
  ...
  // Load projects & details on mount
  useEffect(() => {
    const loadData = async () => {
      ...
      if (id) {
        try {
          ...
          if (data) {
            setProjectId(data.projectId || "all");
            setTitle(data.title || "");
            setCategory(data.category || "GENERAL");
            setBody(data.body || "");
            setPriority(data.priority || "NORMAL");
            setAudienceRoles(data.audienceRoles || ["RESIDENT", "GUEST"]);
            setPinned(!!data.pinned);
            setPublishNow(!!data.publishNow);
            if (data.startsAt) {
              const startD = new Date(data.startsAt);
              setStartsAt(new Date(startD.getTime() - startD.getTimezoneOffset() * 60000).toISOString().slice(0, 16));
            }
            if (data.expiresAt) {
              const expireD = new Date(data.expiresAt);
              setExpiresAt(new Date(expireD.getTime() - expireD.getTimezoneOffset() * 60000).toISOString().slice(0, 16));
            }
            setImageUrls(data.imageUrls || []);
            setAttachmentUrls(data.attachmentUrls || []);
          }
        } catch (error) {
          console.warn("Failed to load announcement details via API:", error);
  ...
  return (
    <FormCard
      ...
      subtitle="Update announcement details and broadcast targets"

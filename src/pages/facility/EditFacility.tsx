...
export default function EditFacility() {
        ...
        try {
          ...
          if (f) {
            ...
            setFormData((prev) => ({
              ...
              price: f.priceLabel || f.price || (f.priceAmount ? `₹${f.priceAmount}/${f.pricingModel?.toLowerCase() === "hourly" ? "hr" : f.pricingModel?.toLowerCase()}` : "Free"),
              capacity: f.capacity?.toString() || "30",
              totalSlots: f.totalSlots?.toString() || "1",
              bookedSlots: f.bookedSlots?.toString() || "0",
              ...
              allDay: (f.openingTime === "00:00" && f.closingTime === "23:59"),
              availableDays: (f.availableDays && f.availableDays.length) ? f.availableDays : prev.availableDays,
              rules: f.rules || "",
              advanceBookingDays: f.advanceBookingDays?.toString() || "7",
              cancellationHours: f.cancellationHours?.toString() || "2",
              imagePreview: f.images?.[0] || "",
              code: f.code || "",
              accessType: f.accessType || "SLOT_BOOKING",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let finalImages = formData.imagePreview && !formData.imageFile ? [formData.imagePreview] : [];
      ...
      toast.error(err?.response?.data?.message || err?.message || "Failed to update facility");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 5 }, bgcolor: "#f8fafc", minHeight: "100vh" }}>
      <Box sx={{ position: "relative", zIndex: 1 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 4, maxWidth: "960px", mx: "auto" }}>

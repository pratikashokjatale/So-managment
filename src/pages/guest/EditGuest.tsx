...
import { getGuestById, updateGuest } from '@/utils/guestStore';
...
export default function EditGuest() {
  ...
  useEffect(() => {
    ...
    // Pre-populate form with existing guest details
    if (id) {
      const guest = getGuestById(id);
      if (guest) {
        setName(guest.name);
        setResidentValue(`${guest.resident}|${guest.apartment}`);
        setFromDate(guest.fromDate);
        setDueDate(guest.dueDate);
        setPurpose(guest.purpose);
        setAddress(guest.address);
        if (guest.aadhaarFile) {
          setUploadedFile({
            name: guest.aadhaarFile,
            size: guest.aadhaarSize || '1.4 MB'
          });
        }
      }
    }
      ...
      <Paper elevation={0} sx={{ border: '1px solid #f0f0f0', borderRadius: 4, p: { xs: 3, md: 5 } }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 5, pt: 3, borderTop: '1px solid #f0f0f0' }}>
          ...
          <Button 
            ...
            sx={{ borderRadius: '10px', textTransform: 'none', fontWeight: 600, px: 4, bgcolor: '#24528C', '&:hover': { bgcolor: '#24528C' } }}
          >
            Save Guest Pass
          </Button>
        </Box>

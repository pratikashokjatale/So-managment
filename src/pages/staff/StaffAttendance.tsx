...
const mapBackendAttendanceToFrontend = (a: any) => {
  const checkIn = a.checkInAt
    ? new Date(a.checkInAt).toLocaleTimeString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "-";
  const checkOut = a.checkOutAt
    ? new Date(a.checkOutAt).toLocaleTimeString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "-";
  ...
  return {
    ...
    shift: a.staff?.shiftStart
      ? `${a.staff.shiftStart} - ${a.staff.shiftEnd}`
      : "General (9 AM - 6 PM)",
    checkIn,
    checkOut,
    status,
    avatar:
      a.staff?.photoUrl || a.staff?.profilePhotoUrl || a.staff?.avatar || "",
    rawLog: a,
  };
};
...
export default function StaffAttendance() {
  ...
  const getStatusChip = (status: string) => {
    switch (status) {
      ...
      case "On Break":
        return (
          <Chip
            icon={<BreakIcon sx={{ fontSize: "16px !important" }} />}
            label="On Break"
            size="small"
            sx={{
              bgcolor: "#eff6ff",
  ...
  const activeLog: LogEntry = selectedRecord
    ? selectedRecord.rawLog
      ? {
          checkInTime: selectedRecord.checkIn,
          checkInStatus:
            selectedRecord.rawLog.status === "LATE"
              ? "Checked In (Late)"
              : "Checked In (On Time)",
          checkInTerminal:
            selectedRecord.rawLog.accessZone || "Main Gate Terminal",
          breakTime: "-",
          breakDuration: "-",
          breakTerminal: "-",
          checkOutTime:
          ...
          checkOutStatus:
            selectedRecord.checkOut === "-"
              ? "Still On Duty / Shift Active"
              : "Checked Out (Shift Completed)",
          checkOutTerminal:
            selectedRecord.checkOut === "-" ? "-" : "Main Exit Terminal",
        }
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" fontWeight="700">
            Real-time Clubhouse crew duty tracking & activity timeline logs
          </Typography>
        </Box>
        <Stack
          direction="row"
                ...
                <TableRow key={row.id} hover>
                  ...
                  <TableCell>
                    <Typography
                      variant="body2"
                      fontWeight="900"
                      color={row.checkIn === "-" ? "#cbd5e1" : "#091542"}
                    >
                      {row.checkIn}
                    </Typography>
                  </TableCell>
                  ...
                  <TableCell>
                    <Typography
                      variant="body2"
                      fontWeight="900"
                      color={row.checkOut === "-" ? "#cbd5e1" : "#091542"}
                    >
                      {row.checkOut}
                    </Typography>
                  </TableCell>
        ...
        <DialogContent sx={{ py: 2 }}>
          <Stack spacing={3}>
            ...
            {selectedRecord && (
              ...
              >
                <Avatar
                  src={getFileUrl(selectedRecord.avatar)}
                  imgProps={{ crossOrigin: "anonymous" }}
                  sx={{
                    width: 56,
                    height: 56,
              ...
              >
                ...
                <Box
                  sx={{
                    ...
                    width: 2,
                    bgcolor:
                      activeLog.checkInTime !== "-" ? "#10b981" : "#e2e8f0",
                    borderStyle:
                      activeLog.checkInTime !== "-" ? "solid" : "dashed",
                  }}
                />
                ...
                >
                  <Box
                    sx={{
                      ...
                      borderRadius: "50%",
                      bgcolor:
                        activeLog.checkInTime !== "-" ? "#f0fdf4" : "#f1f5f9",
                  ...
                  >
                    <LoginIcon
                      sx={{
                        fontSize: 16,
                        color:
                          activeLog.checkInTime !== "-" ? "#10b981" : "#94a3b8",
                      }}
                    />
                  </Box>
                </Box>
              ...
              >
                ...
                <Box
                  sx={{
                    ...
                    width: 2,
                    bgcolor:
                      activeLog.breakTime !== "-" ? "#f59e0b" : "#e2e8f0",
                    borderStyle:
                      activeLog.breakTime !== "-" ? "solid" : "dashed",
                  }}
                />
                ...
                >
                  <Box
                    sx={{
                      ...
                      borderRadius: "50%",
                      bgcolor:
                        activeLog.breakTime !== "-" ? "#fffbeb" : "#f1f5f9",
                  ...
                  >
                    <BreakIcon
                      sx={{
                        fontSize: 16,
                        color:
                          activeLog.breakTime !== "-" ? "#f59e0b" : "#94a3b8",
                      }}
                    />
                  </Box>
                </Box>
